...
def _do_convert(pth_path: str, output_tflite: str, shape=(256, 256)) -> str:
    ...
    try:
        ...
        # Phase 3: Export SavedModel via onnx2tf, then perform Full INT8 Quantization
        ...
        print(f"🔄 [CONVERT] Running onnx2tf conversion with SavedModel export...")
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode != 0:
            err_msg = result.stderr if result.stderr else result.stdout
            print(f"❌ [CONVERT] onnx2tf failed: {err_msg[-500:]}")
            raise RuntimeError(f"onnx2tf conversion failed: {err_msg[-300:]}")

        # Phase 4: Full INT8 Quantization via TFLiteConverter
        import tensorflow as tf
        calib_arr = np.load(temp_calib)
        def rep_data_gen():
            for idx in range(len(calib_arr)):
                yield [np.expand_dims(calib_arr[idx], axis=0).astype(np.float32)]

        print(f"⚙️ [CONVERT] Running TFLiteConverter INT8 Quantization (Full IO int8)...")
        ...
        with open(output_tflite, "wb") as fp:
            fp.write(tflite_bytes)
        print(f"🎉 [CONVERT] Successfully generated INT8 TFLite model at {output_tflite} ({len(tflite_bytes)/(1024*1024):.2f} MB)")

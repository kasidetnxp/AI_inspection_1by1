...
def parse_args():
    parser = argparse.ArgumentParser(description="Analyze model training trends and extrapolate dataset needs.")
...
def main():
    ...
    for fraction in sorted(args.subsets):
        ...
        print(f"Results for size {subset_size}:")
    ...
    report_lines = [
        "="*60,
        "         LEARNING CURVE AND DATA SCALING REPORT",
        "="*60,
    ...
    report_lines.append("\nEXTRAPOLATION AND SAMPLE SIZE PROJECTIONS")
    report_lines.append("Estimate of the number of training images needed to reach target metric thresholds:")
    report_lines.append("-"*60)
    report_lines.append(f"{'Target Metric':<15} | {'Required for Conf':<18} | {'Required for mIoU':<18} | {'Required for Acc':<18}")
    report_lines.append("-"*60)
    
    for t in targets:
        proj = projections[f"target_{t:.3f}"]
        c_val = str(proj['confidence']) if proj['confidence'] else "N/A (flat trend)"
        m_val = str(proj['miou']) if proj['miou'] else "N/A (flat trend)"
        a_val = str(proj['accuracy']) if proj['accuracy'] else "N/A (flat trend)"
        report_lines.append(f"{t*100:<13.1f}% | {c_val:<18} | {m_val:<18} | {a_val:<18}")
    report_lines.append("-"*60)
    
    report_content = "\n".join(report_lines)
    print("\n" + report_content)
    
    with open(os.path.join(OUTPUT_DIR, "learning_curve_report.txt"), "w") as f:
        f.write(report_content)
    print(f"\nSaved text report to: {os.path.join(OUTPUT_DIR, 'learning_curve_report.txt')}")
    
    # 5. Plot curves
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    
    # Prepare interpolation range for plotting
    max_size = max(sample_sizes)
    plot_sizes = np.linspace(min(sample_sizes), max_size * 5, 200)
    
    # Left Plot: Accuracy & mIoU
    axes[0].scatter(sample_sizes, [a*100 for a in accuracies], color="#0078FF", label="Measured Accuracy", zorder=3)
    if acc_pred_fn:
        axes[0].plot(plot_sizes, [acc_pred_fn(n)*100 for n in plot_sizes], color="#0078FF", linestyle="--", label="Accuracy Trend")
        
    axes[0].scatter(sample_sizes, [m*100 for m in mious], color="#E67E22", label="Measured mIoU", zorder=3)
    if miou_pred_fn:
        axes[0].plot(plot_sizes, [miou_pred_fn(n)*100 for n in plot_sizes], color="#E67E22", linestyle="--", label="mIoU Trend")
        
    axes[0].set_xlabel("Number of Training Images (N)")
    axes[0].set_ylabel("Metric Value (%)")
    axes[0].set_title("Accuracy & mIoU Scaling Trend")
    axes[0].grid(True, linestyle=":", alpha=0.6)
    axes[0].legend(loc="lower right")
    
    # Right Plot: Confidence
    axes[1].scatter(sample_sizes, confidences, color="#2ECC71", s=50, label="Measured Confidence", zorder=3)
    if conf_pred_fn:
        axes[1].plot(plot_sizes, [conf_pred_fn(n) for n in plot_sizes], color="#2ECC71", linestyle="--", label="Confidence Trend")
        
    axes[1].set_xlabel("Number of Training Images (N)")
    axes[1].set_ylabel("Mean Confidence (Softmax Probability)")
    axes[1].set_title("Mean Prediction Confidence Scaling Trend")
    axes[1].grid(True, linestyle=":", alpha=0.6)
    
    # Draw horizontal target lines and label them on confidence plot
    colors = ["#9B59B6", "#E74C3C", "#34495E"]
    for i, t in enumerate([0.95, 0.98, 0.99]):
        axes[1].axhline(y=t, color=colors[i], linestyle=":", alpha=0.8, label=f"Target {t*100:.0f}%")
        proj_val = projections[f"target_{t:.3f}"]['confidence']
        if proj_val and proj_val <= max_size * 5:
            axes[1].axvline(x=proj_val, color=colors[i], linestyle=":", alpha=0.8)
            
    axes[1].legend(loc="lower right")
    
    plt.tight_layout()
    plot_path = os.path.join(OUTPUT_DIR, "learning_curve_plot.png")
    plt.savefig(plot_path, dpi=150)
    plt.close()
    print(f"Saved trend visualization plot to: {plot_path}")
    print("\nTrend analysis simulation finished successfully!")

"""Standard U-Net prediction script (PyTorch)."""
import os
import sys
...
MODEL_PATH = get_path("unet", "models")
...
def save_comparison_plot(original_pil, predicted_mask_pil, filename, output_dir):
    """Save side-by-side comparison of original image and predicted mask."""
...
def process_single_image(image_path, model, device, output_dir):
    ...
    # LabelMe JSON saving disabled for production pipeline performance


    # Extract polygons for generic inspection format
    pads, probemarks, grains = [], [], []
    
    # 1. Build solid Pad mask by merging pad (class 1), probemark (class 2), and grain (class 3)
    pad_mask_combined = ((prediction_mask_orig == 1) | (prediction_mask_orig == 2) | (prediction_mask_orig == 3)).astype(np.uint8) * 255
    if np.sum(pad_mask_combined) > 0:
        contours, _ = cv2.findContours(pad_mask_combined, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        for contour in contours:
            if cv2.contourArea(contour) > 500: # Filter out tiny noise
                hull = cv2.convexHull(contour).astype(np.int32)
                pads.append(hull)

    # 2. Extract Probemark and Grain contours
    for class_id, label in ID_TO_LABEL.items():
        if label == "pad" or label == "background":
            continue
...
def main(input_path, out_subfolder=None):
    ...
    run_output_dir = os.path.join(PROJECT_ROOT, "outputs/unet", subfolder_name)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        target_path, out_subfolder = interactive_select_paths("unet", default_dir)

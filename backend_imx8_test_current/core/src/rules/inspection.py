...
def run_inspection(image_results,
    ...
    csv_exists = os.path.exists(output_csv_path) if output_csv_path else False
    results_logged = []
    ...
    for r in image_results:
        ...
        if len(pads) == 0:
            ...
            for pm in probemarks:
                ...
                if best_pad is not None and (MIN_OVERLAP_PCT <= 0.0 or overlap_ratio >= MIN_OVERLAP_PCT):
                    pad_to_pms[best_pad_idx].append(pm)
                elif len(pads) > 0 and MIN_OVERLAP_PCT <= 0.0:
                    # Probemark with 0 overlap to any pad (e.g. completely outside/off-pad)
                    # Assign to closest pad so it is evaluated by distance check (fails distance check -> FAIL)
                    closest_pad_idx = 0
                    min_dist_sq = float('inf')
                    M_pm = cv2.moments(pm)
                    pm_cx = int(M_pm["m10"] / M_pm["m00"]) if M_pm["m00"] > 0 else int(pm[0][0][0])
                    pm_cy = int(M_pm["m01"] / M_pm["m00"]) if M_pm["m00"] > 0 else int(pm[0][0][1])
                    for pad_idx, pad in enumerate(pads):
                        M_pad = cv2.moments(pad)
                        p_cx = int(M_pad["m10"] / M_pad["m00"]) if M_pad["m00"] > 0 else int(pad[0][0][0])
                        p_cy = int(M_pad["m01"] / M_pad["m00"]) if M_pad["m00"] > 0 else int(pad[0][0][1])
                        d = (pm_cx - p_cx) ** 2 + (pm_cy - p_cy) ** 2
                        if d < min_dist_sq:
                            min_dist_sq = d
                            closest_pad_idx = pad_idx
                    pad_to_pms[closest_pad_idx].append(pm)
                else:
                    print(
        ...
        viz_filename = f"{viz_prefix}{image_name}" if viz_prefix else image_name
